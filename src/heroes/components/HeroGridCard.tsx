...
export const HeroGridCard = ({hero}: Props) => {
  ...
  const {taggleFavorites, isFavorite} = use(FavoriteHEroContext)



  const handleHeroById = () =>{
      navegate(`/heroes/${hero.slug}`);
  }


  return (
        <Card className="group overflow-hidden hover:shadow-xl transition-all duration-300 hover:-translate-y-1 bg-gradient-to-br from-white to-gray-50">
            <div className="relative h-64" key={hero.id}>
              <img
                src={hero.image}
                alt={hero.name}
                onClick={handleHeroById}
